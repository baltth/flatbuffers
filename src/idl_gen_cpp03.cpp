...
namespace cpp {
class Cpp03Generator : public BaseGenerator {
 public:
  Cpp03Generator(const Parser &parser, const std::string &path,
               const std::string &file_name)
 ...
 private:
  ...
  void GenParam(const FieldDef &field, bool direct, const char *prefix) {
    code_ += GenParamType(field, direct, prefix) + field.name + " = " + GenParamValue(field, direct);
  }

  std::string GenParamType(const FieldDef &field, bool direct, const char *prefix) {
    std::string code;
    if(prefix != nullptr) {
        code += prefix;
    }
  ...
  std::string GenParamName(const FieldDef &field, const char *prefix) {
    std::string code(field.name);
    if(prefix != nullptr) {
        code = prefix + code;
    }
    return code;
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (field.key) {
        ...
        if (is_string) {
          ...
          code_ += "    const {{KEY_TYPE}} key = {{FIELD_NAME}}();";
          code_ += "    if (key < val) {";
          code_ += "      return -1;";
  ...
  void GenBuilders(const StructDef &struct_def) {
    ...
    code_ += "    const flatbuffers::uoffset_t end = fbb_.EndTable(start_, " + num_fields + ");";
  ...
  std::string GenItemCreatorName(const FieldDef& field) {
    return "ItemCreator_" + field.name;
  }

  void GenCreateParamNestedType(const FieldDef &field) {
    switch (field.value.type.base_type) {
      case BASE_TYPE_VECTOR: {
  ...
  void GenTablePost(const StructDef &struct_def) {
    ...
    if (parser_.opts.generate_object_based_api) {
      ...
      for (auto it = struct_def.fields.vec.begin();
        ...
        code_.SetValue("FIELD_NAME", field.name);
        code_.SetValue("FIELD_TYPE", GenTypeGet(field.value.type, "", "const ", " *", true));
      ...
      for (auto it = struct_def.fields.vec.begin();
        ...
        code_ += "  " + GenParamType(field, false, nullptr) + "_" + field.name + " = " + GenCreateParam(field) + ";";
      }
      // Need to call "Create" with the struct namespace.
      const auto qualified_create_name = struct_def.defined_namespace->GetFullyQualifiedName("Create");
      code_.SetValue("CREATE_NAME", TranslateNameSpace(qualified_create_name));
...
bool GenerateCPP03(const Parser &parser, const std::string &path,
                   const std::string &file_name) {
  cpp::Cpp03Generator generator(parser, path, file_name);
  return generator.generate();
}

std::string CPP03MakeRule(const Parser &parser, const std::string &path,
                        const std::string &file_name) {
  const auto filebase =
